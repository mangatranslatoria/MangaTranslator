...
function setAppPreviewForTheme(theme) {
  ...
  if (badge) badge.textContent = chosen === 'dark' ? 'Dark Theme' : 'Light Theme';
}
...
function updateBrandLogos(theme){
  const src = theme === 'dark' ? 'assets/logo/app-icon-dark.png' : 'assets/logo/app-icon-light.png';
  document.querySelectorAll('.brand-logo').forEach(img=>{ if(img.getAttribute('src')!==src) img.src=src; });
  const fav=document.querySelector('link[rel="icon"]'); if(fav) fav.href = theme === 'dark' ? 'assets/logo/app-icon-dark.ico' : 'assets/logo/app-icon-light.ico';
}
